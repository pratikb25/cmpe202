...
class GumballMachine25Cent extends GumballMachine {
	public GumballMachine25Cent(int nGumballs) {
		super(nGumballs);
	}
	...
	public void releaseBall() {
		if (count != 0) {
			if(inputCoinValue >= 25) {
				int nGumballs = (inputCoinValue / 25);
				if(nGumballs <= count) {
					count -= nGumballs;
					inputCoinValue -= (nGumballs * 25);
